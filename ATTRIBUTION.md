# Attribution

Masterpiece is a player. The instruments and the performances heard through it
were produced by third parties and are credited here.

None of the material listed on this page is included in this repository or
redistributed with the software. Each item links to its source.

---

## Sample libraries

The consoles shown in this repository and the organs heard in the
demonstration recording are freely published sample libraries. Each remains
the property of its producer and is governed by that producer's terms.

| Organ | Instrument | Specification | Producer |
|---|---|---|---|
| [Friesach](https://piotrgrabowski.pl/friesach/) | Eisenbarth, 2000 | 44 stops, III/P | Piotr Grabowski |
| [Nancy](https://piotrgrabowski.pl/nancy/) (demonstration set) | — | 65 stops, IV/P | Piotr Grabowski |
| [Kraków, St. John Cantius](https://piotrgrabowski.pl/cracow-st-john-cantius/) | — | 40 stops, III/P | Piotr Grabowski |
| [Lipiny](https://piotrgrabowski.pl/lipiny/) | A. Volkmann, 1898 | 25 stops, II/P | Piotr Grabowski |
| [Długa Kościelna](https://piotrgrabowski.pl/dluga-koscielna/) | — | 22 stops, II/P | Piotr Grabowski |
| [Raszczyce](https://piotrgrabowski.pl/raszczyce/) | Vermeulen, 1965 | 21 stops, III/P | Piotr Grabowski |
| [Strassburg](https://piotrgrabowski.pl/strassburg/) | C. Werner, 1743 | 20 stops, II/P | Piotr Grabowski |
| [Azzio](https://piotrgrabowski.pl/azzio/) | Mascioni, 2016 | 12 stops, II/P | Piotr Grabowski |
| [Melcer Chamber Music Hall](https://piotrgrabowski.pl/melcer-chamber-music-hall/) | — | chamber organ | Piotr Grabowski |
| [Lemmer](https://hauptwerk-augustine.info/Lemmer.php) | Flentrop, 1977–78 | 9 registers, I/P | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Giubiasco](https://piotrgrabowski.pl/giubiasco/) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Alessandria](https://piotrgrabowski.pl/alessandria/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Pinerolo](https://piotrgrabowski.pl/pinerolo/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Ermelo](https://piotrgrabowski.pl/ermelo/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Święta Lipka](https://piotrgrabowski.pl/swieta-lipka/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Nitra](https://piotrgrabowski.pl/nitra/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Erfurt-Büßleben](https://piotrgrabowski.pl/erfurt-bussleben/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Erfurt, Predigerkirche](https://piotrgrabowski.pl/erfurt-predigerkirche/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Obervellach](https://piotrgrabowski.pl/obervellach/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Düren](https://piotrgrabowski.pl/duren/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Chorzów, St Hedwig of Silesia](https://piotrgrabowski.pl/chorzow-sw-jadwiga-slaska/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Goch](https://piotrgrabowski.pl/goch/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Nagyrákos](https://hauptwerk-augustine.info/Angster_Nagyrakos.php) (demonstration set) | Angster, 1938 | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Hungarian village organs](https://hauptwerk-augustine.info/Hungarian_village.php) (composite set) | — | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Zalalövő](https://hauptwerk-augustine.info/Zalalovo.php) (demonstration set) | Aquincum | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Császár](https://hauptwerk-augustine.info/Luber_Csaszar.php) (demonstration set) | F. X. Luber, 1793 | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Vasvár, Dominican church](https://hauptwerk-augustine.info/Vasvar.php) (demonstration set) | — | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Kézdimartonfalva](https://hauptwerk-augustine.info/Dutch_harmonium.php) | harmonium | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Szombathely](https://hauptwerk-augustine.info/Eule_Szombathely.php) (demonstration set) | Eule | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Hajós](https://hauptwerk-augustine.info/Angster_Hajos.php) (demonstration set) | Angster | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Székesfehérvár, Franciscan church](https://hauptwerk-augustine.info/Mauracher_Fehervar.php) (demonstration set) | Mauracher | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Székesfehérvár, Cistercian church](https://hauptwerk-augustine.info/Angster__Cistercian.php) (demonstration set) | Angster | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Ják Abbey](https://hauptwerk-augustine.info/Angster_Jak.php) (demonstration set) | Angster, 1902 | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Monor, Reformed church](https://hauptwerk-augustine.info/Rieger_Monor.php) (demonstration set) | Rieger | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Budapest, Church of the Holy Spirit](https://hauptwerk-augustine.info/Holy_Spirit.php) (demonstration set) | — | — | [Augustine's Virtual Organs](https://hauptwerk-augustine.info/) |
| [Oloron-Sainte-Marie](https://piotrgrabowski.pl/oloron-sainte-marie/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |
| [Bégard](https://piotrgrabowski.pl/begard/) (demonstration set) | — | — | [Piotr Grabowski](https://piotrgrabowski.pl/) |

---

The second recording, *New tested instruments* (thirty works on fifteen
organs, filmed from the running console), has its own programme and
sequencer credits:
[attribution for New tested instruments](https://bonninr.github.io/masterpiece/attribution-2.html).

## Music

The demonstration recording is played from MIDI sequences produced for the
Sound Canvas Pipe Organ Project (SCPOP). Each sequence is a performance; the
phrasing and registration changes are the sequencer's work.

Archive: [organosandomenicorieti.it/scpop](https://www.organosandomenicorieti.it/scpop/archive.htm).
The Widor sequences were obtained via
[classicalmidi.co.uk](https://classicalmidi.co.uk/charlesmariewidor.htm).

> **Licence note.** Most of these files carry the notice *"Commercial use of
> this file is not allowed."* The demonstration recording is published on those
> terms: freely viewable, not sold, and not licensed for commercial use.

| Work | Sequenced by |
|---|---|
| Bach — Toccata and Fugue in D minor, BWV 565 | Raffaele Diodati, 1997–98 |
| Bach — Prelude and Fugue in A minor, BWV 543 | Andrew Maguire, 2000 |
| Bach — Toccata, Adagio and Fugue in C, BWV 564 | Andrew Maguire |
| Bach — Wachet auf, ruft uns die Stimme, BWV 645 | Raffaele Diodati |
| Bach — Ich ruf zu dir, Herr Jesu Christ, BWV 639 | Raffaele Diodati |
| Widor — Toccata, Symphony no. 5 | William Shattuck |
| Widor — Symphony no. 1: Marche Pontificale | F. Borsari |
| Reger — Fantasia on 'Ein feste Burg' | not named in the file |
| Reger — Three chorale preludes, op. 135a | Raffaele Diodati |
| Boëllmann — Suite Gothique | Andrew Maguire, 1999 |
| Grison — Toccata in F | Ron Pearcy |
| Karg-Elert — Nun danket alle Gott | Andrew Maguire |
| Buxtehude — Prelude and Fugue in G minor | Andrew Maguire, 1998 |
| Buxtehude — Praeludium, Fuga and Ciacona in C | Henk Beens |
| Buxtehude — Nun komm, der Heiden Heiland | Andrew Maguire, 1999 |
| Brahms — Prelude and Fugue in G minor | F. Borsari |
| Brahms — Herzlich tut mich verlangen | Raffaele Diodati, 1997–98 |
| Mozart — Fantasia in F minor, K608 | Raffaele Diodati |
| Zipoli — Toccata in D minor | Raffaele Diodati, 1998 |
| Zipoli — Pastorale | Raffaele Diodati |
| Pasquini — Toccata con lo scherzo del cucù | Raffaele Diodati |
| Franck — Chorale no. 3 in A minor | Raffaele Diodati |
| Franck — Prière, op. 20 | Raffaele Diodati |
| Telemann — Fantasia in D | Henk Beens |
| Stanley — Voluntary in A minor | Louke B. Kleyn, 1998 |
| Pachelbel — Theme and Variations | Ray Huggett |
| Banchieri — Dialogo per organo | Ignacio Sniechowski |
| Banchieri — Fantasia IV | Gheller; SCPOP arrangement by Raffaele Diodati |
| Frescobaldi — Capriccio sopra il Cucù | Henk Beens |
| Sweelinck — Puer nobis nascitur, variations | Henk Beens |
| Bruckner — Cantabile | Andrea Amici |
| Byrd — Pavane | Raffaele Diodati, 2001 |

All of the above carry the credit *"Organ sounds by The Sound Canvas Pipe Organ
Project — www.scpop.de"*. Excerpts are used, not complete works.

### Adaptation

The sequences were written for a Roland Sound Canvas and required four changes
before a sampled organ could play them:

- **Channel assignment.** SCPOP places the manuals on channels 1 and 2 and the
  pedal on channel 3; the sequences are remapped to the key flow a sampled
  organ expects.
- **Pedal compass.** Notes below a physical pedalboard's range are folded into
  it, and the octave doubling used to add weight on the Sound Canvas is
  removed, since a 32′ or 16′ stop supplies it.
- **Registration.** The system-exclusive messages that selected Sound Canvas
  voices have no meaning here; registration is drawn on the instrument.
- **Ties.** Where a sequence releases the notes of an arpeggiated chord as the
  next note arrives and the score ties them, the notes are sustained. The two
  diminished sevenths in BWV 565 are the principal case.

---

## Editions

Score readings were checked against the
[Mutopia Project](https://www.mutopiaproject.org/) edition of BWV 565, typeset
in LilyPond and released to the public domain.

---

## Software

The published work of these projects was a reference and a source of
understanding throughout, and is gratefully acknowledged:
[GrandOrgue](https://github.com/GrandOrgue/GrandOrgue),
[OdfEdit](https://github.com/GrandOrgue/OdfEdit),
[rusty-pipes](https://github.com/dividebysandwich/rusty-pipes) and
[HISE](https://github.com/christophhart/HISE).

It is built on [JUCE](https://juce.com/), [pugixml](https://pugixml.org/) and
[libarchive](https://www.libarchive.org/), which reads organs straight from their RAR
packages (BSD licence).

Masterpiece is not affiliated with, nor endorsed by, any sample library
producer or any other organ software.

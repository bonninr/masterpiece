...
juce::Image woodTileFor(const std::string& name) {
  ...
  const bool vertical = n.containsIgnoreCase("VGrain") || n.containsIgnoreCase("VerticalGrain");
  ...
  if (!img.isValid() && set.packageId > 0 && set.packageId <= 10) {
    const std::string& name =
        element->bitmapFile.find_first_of("0123456789") != std::string::npos
            ? element->bitmapFile
            : set.name;
    // Only the seamless backgrounds. "Button06-Wood-Large-Off" is a wooden
    // stop button, and tiling wood in its place buried the harmonium's
    // console under 256-pixel planks.
    if (juce::String(name).containsIgnoreCase("seamless")) {
